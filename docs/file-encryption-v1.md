...
这是 ToolBox 自定义容器，不是第三方审计过的通用加密文件标准。安全依赖密码强度、平台 provider、设备安全以及原文件头的公开策略，不保证隐藏类型、大小或文件名。
...
- **不兼容 TheBook 的旧容器**。参考了其大文件分块处理思路，但没有复制固定口令、固定 IV 或 MD5 派生。迁移时先用 TheBook 解密，再由 ToolBox 重新加密。
